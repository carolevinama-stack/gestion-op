...
const I = {
  ...
  journal: (c, s=20) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/><line x1="9" y1="8" x2="15" y2="8"/><line x1="9" y1="12" x2="15" y2="12"/></svg>,
...
const menuConfig = [
  ...
  { isDivider: true },
  { 
    id: 'grp_operations', icon: 'operations', label: 'Opérations',
    subItems: [
  ...
  { 
    id: 'circuit', icon: 'circuit', label: 'Circuit Validation',
    subItems: [
  ...
  { 
    id: 'grp_gestion', icon: 'gestion', label: 'Gestion',
    subItems: [
  ...
  { 
    id: 'grp_config', icon: 'config', label: 'Configuration',
    subItems: [
  ...
  return (
    <div onClick={() => onClick(id)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      style={{
        ...
        height: isSubItem ? 38 : 44, borderRadius: 12, cursor: 'pointer',
      ...
      <div style={{
        ...
        background: active && !isSubItem ? 'rgba(255,255,255,0.18)' : 'transparent', transition: 'all 0.2s ease',
      }}>
        {I[icon]?.(active ? '#fff' : isH ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.5)', isSubItem ? 16 : 20)}
      </div>
      {!collapsed && (
        <span style={{ fontSize: isSubItem ? 12.5 : 13.5, fontWeight: active ? 700 : 500, color: active ? '#fff' : isH ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.6)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', transition: 'color 0.15s', flex: 1 }}>{label}</span>
      )}
...
const Sidebar = () => {
  ...
  // LOGIQUE CRUCIALE : Ouvrir automatiquement le groupe qui contient la page active
  useEffect(() => {
  ...
  const items = useMemo(() => {
    ...
    const withAdmin = userProfile?.role !== 'ADMIN'
      ...
      : menuConfig.map(item =>
          item.id === 'grp_config'
            ? { ...item, subItems: [...item.subItems, { id: 'admin', icon: 'admin', label: 'Administration' }, { id: 'journal', icon: 'journal', label: 'Journal' }] }
            : item
        );
  ...
  return (
    <div style={{
      ...
      <button onClick={() => setCollapsed(c => !c)} style={{ position: 'absolute', top: 28, right: -14, width: 28, height: 28, borderRadius: '50%', border: 'none', background: PRIMARY, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 20, boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }} title={collapsed ? 'Déplier' : 'Replier'}>
        <span style={{ display: 'flex', transform: collapsed ? 'rotate(180deg)' : 'none', transition: 'transform 0.25s' }}>{I.chevron('#fff', 16)}</span>
      </button>
      ...
      <nav className="modern-scroll" style={{ flex: 1, padding: '4px 0', overflowY: 'auto', overflowX: 'hidden' }}>
        {items.map((item, idx) => {
          if (item.isDivider) {
            return <div key={'s'+idx} style={{ margin: collapsed ? '15px 0' : '20px 0 10px 0' }}><div style={{ height: 1, background: 'linear-gradient(90deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.2) 50%, rgba(255,255,255,0) 100%)', margin: '0 20px' }}/></div>;
          }
          if (item.subItems) {
            ...
            return (
              <div key={item.id}>
                <NavItem id={item.id} icon={item.icon} label={item.label} active={isActiveGroup} collapsed={collapsed} onClick={() => handleGroupClick(item.id)} hasChildren isOpen={isOpen} />
                {!collapsed && isOpen && (
                  <div style={{ marginLeft: 30, paddingLeft: 12, borderLeft: '1px solid rgba(255,255,255,0.15)', marginTop: 4, display: 'flex', flexDirection: 'column', gap: 4, animation: 'fadeIn 0.2s ease forwards' }}>
                    {item.subItems.map(sub => <NavItem key={sub.id} id={sub.id} icon={sub.icon} label={sub.label} active={currentPage === sub.id} collapsed={false} onClick={setCurrentPage} isSubItem />)}
                  </div>
                )}
              </div>
            );
      ...
      {exerciceActif && (
        <div style={{ padding: collapsed ? '12px 0' : '12px 22px', textAlign: collapsed ? 'center' : 'left' }}>
          {collapsed ? <div style={{ fontSize: 14, fontWeight: 800, color: '#E8B931' }}>{String(exerciceActif.annee).slice(-2)}</div> : <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: '#E8B931', boxShadow: '0 0 6px rgba(232,185,49,0.4)' }}/><span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>Exercice</span><span style={{ fontSize: 14, fontWeight: 800, color: '#fff' }}>{exerciceActif.annee}</span></div>}
        </div>
      )}
      ...
      <div style={{ padding: collapsed ? '14px 0' : '14px 14px', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: 10, alignItems: collapsed ? 'center' : 'stretch' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: collapsed ? 'center' : 'flex-start' }}>
          <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(255,255,255,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>{(user?.email || 'U')[0].toUpperCase()}</div>
          {!collapsed && <div style={{ overflow: 'hidden', flex: 1 }}><div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.85)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{user?.email}</div></div>}
        </div>
        {!collapsed ? <button onClick={handleLogout} style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.08)', fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.5)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center' }}>{I.logout('rgba(255,255,255,0.5)')} Déconnexion</button> : <button onClick={handleLogout} title="Déconnexion" style={{ width: 38, height: 34, borderRadius: 8, margin: '0 auto', border: 'none', background: 'rgba(255,255,255,0.08)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{I.logout('rgba(255,255,255,0.5)')}</button>}
      </div>
    </div>
  );
};
