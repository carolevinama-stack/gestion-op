...
const ROLES = {
  ADMIN: { label: 'Administrateur', color: '#c62828', bg: '#ffebee', icon: '👑', description: 'Accès total + gestion utilisateurs' },
  SAISIE: { label: 'Saisie OP', color: '#e65100', bg: '#fff3e0', icon: '✏️', description: 'Créer, modifier, supprimer les OP' },
  CF: { label: 'Contrôleur Financier', color: '#1565c0', bg: '#e3f2fd', icon: '✅', description: 'Viser, différer, rejeter les OP' },
  AC: { label: 'Agent Comptable', color: '#2e7d32', bg: '#e8f5e9', icon: '💳', description: 'Payer et archiver les OP' },
  CONSULTATION: { label: 'Consultation', color: '#616161', bg: '#f5f5f5', icon: '👁️', description: 'Voir sans modifier' }
};
...
const PageAdmin = () => {
  ...
  const [createForm, setCreateForm] = useState({
    email: '', nom: '', role: 'SAISIE', password: ''
  });
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      setCreateForm({ email: '', nom: '', role: 'SAISIE', password: '' });
      await loadUsers();
    } catch (error) {
  ...
  const handleToggleActive = async (userDoc) => {
    ...
    try {
      ...
      showMessage(newActif ? '✅ Compte réactivé' : '⚠️ Compte désactivé');
    } catch (error) {
      alert('❌ Erreur: ' + error.message);
    }
  };
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button 
          onClick={() => {
            setCreateForm({ email: '', nom: '', role: 'SAISIE', password: generateTempPassword() });
                  ...
                  <tr key={u.id} style={{ borderTop: '1px solid #eee', opacity: u.actif === false ? 0.5 : 1 }}>
                    ...
                    <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                      <span style={{
                        ...
                        color: u.actif === false ? '#c62828' : '#2e7d32'
                      }}>
                        {u.actif === false ? '🔴 Inactif' : '🟢 Actif'}
                      </span>
                      {u.mustChangePassword && (
                        <div style={{ fontSize: 10, color: '#f57f17', marginTop: 4 }}>⚠️ Doit changer son MDP</div>
                      )}
                    </td>
                    <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap' }}>
                        ...
                        {!isCurrentUser && (
                          <button 
                            ...
                            style={{ padding: '5px 10px', border: '1px solid #ffcdd2', borderRadius: 6, background: '#fff5f5', cursor: 'pointer', fontSize: 11, color: '#c62828' }}
                            title="Supprimer"
                          >
                            🗑️
                          </button>
