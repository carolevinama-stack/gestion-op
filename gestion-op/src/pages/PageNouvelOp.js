...
const PageNouvelOp = () => {
  ...
  const handleSave = async () => {
    ...
    if (form.modeReglement === 'VIREMENT' && !selectedRib) { alert('Veuillez renseigner un RIB pour le bénéficiaire (mode virement).'); return; }
    ...
    try {
      ...
      setToast(`✅ OP ${opData.numero} créé avec succès !`);
    ...
    <div>
      ...
      {toast && (
        <div style={{
          ...
          background: '#1b5e20', color: 'white', padding: '18px 36px', borderRadius: 12,
          fontSize: 15, fontWeight: 700, zIndex: 9999, boxShadow: '0 8px 32px rgba(0,0,0,0.3)',
          pointerEvents: 'none'
        }}>
          {toast}
        </div>
