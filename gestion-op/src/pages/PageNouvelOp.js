...
const PageNouvelOp = () => {
  ...
  const defaultForm = { type: 'PROVISOIRE', beneficiaireId: '', ribIndex: 0, modeReglement: 'VIREMENT', objet: '', piecesJustificatives: '', montant: '', ligneBudgetaire: '', montantTVA: '', tvaRecuperable: false, opProvisoireNumero: '', opProvisoireId: '' };
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', background: 'white', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderTop: `3px solid ${accent}` }}>
          <div style={{ padding: '24px 28px 20px' }}>
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ ...labelStyle, marginBottom: 10 }}>TYPE D'OP *</label>
                ...
                {[
                  ...
                  return (
                    <button key={type.value} type="button"
                      ...
                      style={{ padding: '10px 18px', borderRadius: 10, border: active ? `2px solid ${type.color}` : '2px solid #e0e0e0', background: active ? type.color + '12' : 'white', color: active ? type.color : '#888', fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          {[{ val: true, lbl: 'OUI' }, { val: false, lbl: 'NON' }].map(opt => {
                            ...
                            return (
                              <div key={opt.lbl} onClick={() => setForm({ ...form, tvaRecuperable: opt.val })}
                                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, border: `1.5px solid ${active ? accent : '#e0e0e0'}`, background: active ? accent + '08' : 'white', cursor: 'pointer' }}>
