        ...
        <div style={{ maxWidth: 900, margin: '0 auto', background: 'white', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderTop: `3px solid ${accent}` }}>
          <div style={{ padding: '24px 28px 20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', marginBottom: 20 }}>
              ...
              <button onClick={handleClear} style={{ padding: '10px 20px', borderRadius: 8, border: '1.5px solid #e0e0e0', background: 'white', fontSize: 12, fontWeight: 600, color: '#666', cursor: 'pointer' }}>EFFACER</button>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <select value={form.type}
                  ...
                  style={{ padding: '10px 14px', borderRadius: 8, border: `1.5px solid ${({ PROVISOIRE: '#ff9800', DIRECT: '#2196f3', DEFINITIF: '#4caf50', ANNULATION: '#f44336' })[form.type]}40`, background: 'white', fontSize: 14, fontWeight: 700, color: ({ PROVISOIRE: '#ff9800', DIRECT: '#2196f3', DEFINITIF: '#4caf50', ANNULATION: '#f44336' })[form.type], cursor: 'pointer', outline: 'none', minWidth: 200 }}>
                  <option value="PROVISOIRE">⏳ Provisoire</option>
                  <option value="DIRECT">⚡ Direct</option>
                  <option value="DEFINITIF">✅ Définitif</option>
                  <option value="ANNULATION">✕ Annulation</option>
                </select>
                <span style={{ padding: '6px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, background: ({ PROVISOIRE: '#ff980015', DIRECT: '#2196f315', DEFINITIF: '#4caf5015', ANNULATION: '#f4433615' })[form.type], color: ({ PROVISOIRE: '#ff9800', DIRECT: '#2196f3', DEFINITIF: '#4caf50', ANNULATION: '#f44336' })[form.type] }}>
                  {form.type}
                </span>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <textarea value={form.objet} onChange={(e) => setForm({ ...form, objet: e.target.value })}
                  style={{ ...editFieldStyle, width: '100%', minHeight: 70, resize: 'vertical', fontFamily: 'inherit', fontSize: 13, outline: 'none' }} placeholder="Décrire l'objet de la dépense..." />
              </div>
              <div>
                ...
                <textarea value={form.piecesJustificatives} onChange={(e) => setForm({ ...form, piecesJustificatives: e.target.value })}
                  style={{ ...editFieldStyle, width: '100%', minHeight: 50, resize: 'vertical', fontFamily: 'inherit', fontSize: 13, outline: 'none' }} placeholder="Lister les pièces jointes..." />
              </div>
            </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr 1fr', gap: 16, alignItems: 'start' }}>
                <div>
                  ...
                  <MontantInput value={form.montant} onChange={(val) => setForm({ ...form, montant: val })}
                    style={{ ...editFieldStyle, fontFamily: 'monospace', fontSize: 16, textAlign: 'right', width: '100%' }} placeholder="0"
                          ...
                          {[{ val: true, lbl: 'OUI' }, { val: false, lbl: 'NON' }].map(opt => {
                            ...
                            return (
                              <div key={opt.lbl} onClick={() => setForm({ ...form, tvaRecuperable: opt.val })}
                                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, border: `1.5px solid ${active ? accent : form.tvaRecuperable === null ? '#ffcc8040' : '#e0e0e0'}`, background: active ? accent + '08' : 'white', cursor: 'pointer' }}>
