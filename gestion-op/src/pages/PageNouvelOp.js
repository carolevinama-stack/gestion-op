  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const opData = {
        ...
        beneficiaireId: form.beneficiaireId, beneficiaireNom: selectedBeneficiaire?.nom || '',
        modeReglement: form.modeReglement,
        rib: selectedRib || null, ligneBudgetaire: form.ligneBudgetaire,
        dotationLigne: getDotation(),
        ...
        tvaRecuperable: form.tvaRecuperable, statut: 'EN_COURS',
              ...
              <div style={{ marginBottom: 20, padding: 16, background: form.type === 'ANNULATION' ? '#ffebee' : '#e8f5e9', borderRadius: 8 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 12, alignItems: 'end' }}>
                  ...
                  <div>
                    ...
                    <Autocomplete
                      options={opProvisoiresDisponibles.map(op => { const ben = beneficiaires.find(b => b.id === op.beneficiaireId); return { value: op.id, label: `${op.numero} - ${ben?.nom || 'N/A'} - ${formatMontant(op.montant)} F`, searchFields: [op.numero, ben?.nom || '', String(op.montant), op.objet || ''] }; })}
                      value={form.opProvisoireId ? opProvisoiresDisponibles.filter(o => o.id === form.opProvisoireId).map(op => { const ben = beneficiaires.find(b => b.id === op.beneficiaireId); return { value: op.id, label: `${op.numero} - ${ben?.nom || 'N/A'} - ${formatMontant(op.montant)} F` }; })[0] || null : null}
