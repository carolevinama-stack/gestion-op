...
const PageConsulterOp = () => {
  ...
  const getEngagementsAnterieurs = () => {
    ...
    return ops
      .filter(op => op.sourceId === activeSource && op.exerciceId === exerciceActif?.id && op.ligneBudgetaire === form.ligneBudgetaire && ['DIRECT', 'DEFINITIF', 'PROVISOIRE'].includes(op.type) && !['REJETE', 'ANNULE'].includes(op.statut))
      .reduce((sum, op) => sum + (op.montant || 0), 0);
  };

  const opProvisoiresDisponibles = ops.filter(op => 
    op.sourceId === activeSource && op.exerciceId === exerciceActif?.id && op.type === 'PROVISOIRE' && !['REJETE', 'ANNULE'].includes(op.statut) &&
  ...
  const handleRejeter = () => {
    ...
    if (motif !== null) {
      updateDoc(doc(db, 'ops', selectedOp.id), { statut: 'REJETE_CF', motifRejet: motif, updatedAt: new Date().toISOString() });
...
<div class="page-container"><div class="inner-frame">
  ...
  // === MODIFICATION N° OP / BORDEREAU (protégée par mot de passe) ===
  ...
  const handleSaveNumero = async () => {
    ...
    if (doublon) { alert('⛔ Ce numéro existe déjà : ' + newNum); return; }
    try {
      const oldNum = selectedOp.numero;
      await updateDoc(doc(db, 'ops', selectedOp.id), { numero: newNum, updatedAt: new Date().toISOString() });
      // Mettre à jour les OP qui référencent l'ancien numéro dans opProvisoireNumero
      ...
      setEditNumero(null);
      alert('✅ Numéro modifié : ' + oldNum + ' → ' + newNum);
        ...
        <div style={{ ...styles.card, borderRadius: '0 0 10px 10px', padding: 0 }}>
          <div style={{ padding: 24 }}>
            ...
            {selectedOp ? (
              <>
                {/* === BLOC RÉFÉRENCE N° OP === */}
                <div style={{ marginBottom: 20, padding: 14, background: '#f8f9fa', borderRadius: 8, border: '1px solid #e9ecef' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 200 }}>
                      ...
                      {editNumero !== null ? (
                        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                          <input value={editNumero} onChange={e => setEditNumero(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleSaveNumero(); if (e.key === 'Escape') setEditNumero(null); }} style={{ ...styles.input, marginBottom: 0, fontSize: 13, fontFamily: 'monospace', fontWeight: 700, padding: '6px 10px', flex: 1 }} autoFocus />
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 14, color: '#1a1a2e' }}>{selectedOp.numero}</span>
                          <button onClick={handleStartEditNumero} title="Modifier le N° OP" style={{ border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 13, padding: '2px 4px', opacity: 0.5 }}>✏️</button>
                        </div>
                      )}
                    </div>

                    {/* Bordereaux (lecture seule) */}
                    {selectedOp.bordereauCF && (
                      <div>
                        <div style={{ fontSize: 9, fontWeight: 700, color: '#6c757d', marginBottom: 4 }}>BORDEREAU CF</div>
                        <span style={{ fontFamily: 'monospace', fontSize: 12, color: '#333' }}>{selectedOp.bordereauCF}</span>
                      </div>
                    )}
                    {selectedOp.bordereauAC && (
                      <div>
                        <div style={{ fontSize: 9, fontWeight: 700, color: '#6c757d', marginBottom: 4 }}>BORDEREAU AC</div>
                        <span style={{ fontFamily: 'monospace', fontSize: 12, color: '#333' }}>{selectedOp.bordereauAC}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {isEditMode ? (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 16, borderTop: '2px solid ' + (currentSourceObj?.couleur || '#1565c0') }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button onClick={handleRejeter} style={{ ...styles.button, padding: '12px 18px', fontSize: 13, background: '#c62828' }}>❌ Rejeter</button>
