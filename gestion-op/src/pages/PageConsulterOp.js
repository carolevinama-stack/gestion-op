...
const PageConsulterOp = () => {
  ...
  const handlePrint = () => {
    ...
    // Pour l'impression : afficher avec signe moins si annulation
    const printMontantTotal = isAnnulation ? ('- ' + formatMontant(engagementActuel)) : formatMontant(engagementActuel);
    const printEngActuel = isAnnulation ? ('- ' + formatMontant(engagementActuel)) : formatMontant(engagementActuel);
    const printEngCumules = isAnnulation
      ? formatMontant(engagementsCumules - engagementActuel)
      : formatMontant(engagementsCumules + engagementActuel);
    const printDisponible = isAnnulation
      ? formatMontant(getDotation() - engagementsCumules + engagementActuel)
      : formatMontant(getDotation() - engagementsCumules - engagementActuel);
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 3fr', gap: 16, alignItems: 'start' }}>
                        <div style={{ gridColumn: '1 / 3', background: P.bgApp, padding: 14, borderRadius: 12, border: `1px solid ${P.bgSection}` }}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '4px 12px' }}>
                            ...
                            <span style={{ fontSize: 12, fontFamily: 'monospace', textAlign: 'right', fontWeight: 600, color: getEngagementActuel() < 0 ? '#2e7d32' : P.orange }}>{getEngagementActuel() >= 0 ? '+' : '-'}{formatMontant(Math.abs(getEngagementActuel()))}</span>
