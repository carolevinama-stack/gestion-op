                        ...
                        return (
                          <div key={op.id} onClick={() => loadOp(op)} style={{ padding: '10px 14px', borderBottom: `1px solid ${P.bgApp}`, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: isSelected ? accent + '0a' : 'transparent', transition: 'background 0.15s' }}
                            ...
                            <div style={{ textAlign: 'right' }}>
                              <div style={{ fontWeight: 700, fontFamily: 'monospace', color: accent, fontSize: 12 }}>{op.type === 'ANNULATION' ? '-' : ''}{formatMontant(op.montant)} F</div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 3fr', gap: 16, marginBottom: 16 }}>
                        <div>
                          ...
                          : <div style={{ ...fieldStyle, fontFamily: 'monospace', fontSize: 16, fontWeight: 700, textAlign: 'right', color: selectedOp.type === 'ANNULATION' ? '#C43E3E' : accent }}>{selectedOp.type === 'ANNULATION' ? '-' : ''}{formatMontant(selectedOp.montant)}</div>}
                        </div>
