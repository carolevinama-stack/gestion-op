      ...
      {!exerciceActif ? (
        ...
        <div style={{ maxWidth: 1020, margin: '0 auto', background: 'white', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderTop: `3px solid ${accent}` }}>
          <div style={{ padding: '24px 28px 20px' }}>
            ...
            {selectedOp ? (
              <>
                {/* Bloc référence : N° OP + TYPE + DATE alignés */}
                  ...
                  <div style={{ flex: '1 1 240px', minWidth: 200 }}>
                    ...
                    {editNumero !== null ? (
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        <input value={editNumero} onChange={e => setEditNumero(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleSaveNumero(); if (e.key === 'Escape') setEditNumero(null); }} style={{ ...fieldStyle, fontSize: 13, fontFamily: 'monospace', fontWeight: 700, background: '#fffde7' }} autoFocus />
                        <button onClick={handleSaveNumero} style={{ border: 'none', background: '#1b5e20', color: '#fff', borderRadius: 6, padding: '8px 12px', fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>✓</button>
                        <button onClick={() => setEditNumero(null)} style={{ border: 'none', background: '#999', color: '#fff', borderRadius: 6, padding: '8px 12px', fontSize: 11, cursor: 'pointer' }}>✕</button>
                      </div>
                    ) : (
                      <div style={{ ...fieldStyle, display: 'flex', alignItems: 'center', gap: 8, height: 38 }}>
                  ...
                  <div style={{ flex: '0 1 180px', minWidth: 160 }}>
                    ...
                    {isEditMode ? (
                      <select value={form.type}
                        ...
                        style={{ ...fieldStyle, fontWeight: 700, fontSize: 14, height: 38,
                          color: (typeColors[form.type] || '#999'),
                          cursor: 'pointer', borderColor: (typeColors[form.type] || '#999') + '40' }}>
                      ...
                      <div style={{ ...fieldStyle, height: 38, display: 'flex', alignItems: 'center' }}>
                        <span style={{ padding: '3px 12px', borderRadius: 6, fontSize: 12, fontWeight: 700, background: (typeColors[selectedOp.type] || '#999') + '15', color: typeColors[selectedOp.type] || '#999' }}>{selectedOp.type}</span>
                      </div>
                    )}
                  </div>
                  <div style={{ flex: '0 1 140px', minWidth: 120 }}>
                    <label style={labelStyle}>DATE</label>
                    <div style={{ ...fieldStyle, height: 38, display: 'flex', alignItems: 'center', fontFamily: 'monospace' }}>{selectedOp.dateCreation || ''}</div>
                  </div>
                  {selectedOp.bordereauCF && (
                    <div style={{ flex: '0 1 auto' }}>
                      <label style={labelStyle}>BRD CF</label>
                      <div style={{ ...fieldStyle, height: 38, display: 'flex', alignItems: 'center', fontFamily: 'monospace', fontSize: 12 }}>{selectedOp.bordereauCF}</div>
                    </div>
                  )}
                  {selectedOp.bordereauAC && (
                    <div style={{ flex: '0 1 auto' }}>
                      <label style={labelStyle}>BRD AC</label>
                      <div style={{ ...fieldStyle, height: 38, display: 'flex', alignItems: 'center', fontFamily: 'monospace', fontSize: 12 }}>{selectedOp.bordereauAC}</div>
                    </div>
                  )}
                  {statutInfo && (
                    <div style={{ flex: '0 1 auto' }}>
                      <label style={labelStyle}>STATUT</label>
                      <div style={{ height: 38, display: 'flex', alignItems: 'center', padding: '0 14px', borderRadius: 8, background: statutInfo.bg, color: statutInfo.color, fontWeight: 700, fontSize: 12 }}>{statutInfo.icon} {statutInfo.label}</div>
                    </div>
                  )}
                </div>

                {/* OP Provisoire pour Annulation/Définitif (en mode édition) */}
                {isEditMode && ['ANNULATION', 'DEFINITIF'].includes(form.type) && (
                  <div style={{ marginBottom: 24, padding: 16, background: form.type === 'ANNULATION' ? '#ffebee' : '#e8f5e9', borderRadius: 10, border: `1.5px solid ${form.type === 'ANNULATION' ? '#ffcdd2' : '#c8e6c9'}` }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 12, alignItems: 'end' }}>
                      <div>
                        ...
                        <input type="text" value={form.opProvisoireNumero} onChange={(e) => setForm({ ...form, opProvisoireNumero: e.target.value, opProvisoireId: '' })} style={{ ...editFieldStyle, height: 38 }} placeholder="Ex: ETAT-2025-0012" />
                      </div>
                ...
                {/* OP Provisoire (lecture seule) */}
                {!isEditMode && selectedOp.opProvisoireNumero && (
                  <div style={{ marginBottom: 16, padding: '10px 16px', background: selectedOp.type === 'ANNULATION' ? '#ffebee' : '#e8f5e9', borderRadius: 8, border: `1.5px solid ${selectedOp.type === 'ANNULATION' ? '#ffcdd2' : '#c8e6c9'}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: selectedOp.type === 'ANNULATION' ? '#c62828' : '#2e7d32' }}>🔄 OP Prov. {selectedOp.type === 'ANNULATION' ? 'annulé' : 'régularisé'} :</span>
                    <span style={{ fontFamily: 'monospace', fontWeight: 600, fontSize: 12 }}>{selectedOp.opProvisoireNumero}</span>
                  </div>
                )}
                ...
                <div style={isReadOnly ? { pointerEvents: 'none' } : {}}>
                  ...
                  {/* Motif rejet — aligné à la largeur du bloc budget */}
                  {selectedOp.motifRejet && (
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 3fr', gap: 16, marginBottom: 24 }}>
                      <div style={{ gridColumn: '1 / 3', padding: 14, background: '#ffebee', borderRadius: 10, border: '1.5px solid #ffcdd2' }}>
