...
const PageConsulterOp = () => {
  ...
  // OP provisoires pour ANNULATION / DEFINITIF
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'end', marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: '0 0 auto', position: 'relative' }} ref={dropdownRef}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
                  <input type="text" value={searchText} onChange={(e) => { setSearchText(e.target.value); setShowDropdown(true); }} onFocus={() => setShowDropdown(true)} placeholder="N° OP, bénéficiaire..." style={{ width: 200, padding: '7px 10px', border: '1.5px solid #e0e0e0', borderRadius: '8px 0 0 8px', outline: 'none', fontFamily: 'monospace', fontWeight: 600, fontSize: 12 }} />
                  <div style={{ display: 'flex', flexDirection: 'column', borderRadius: '0 8px 8px 0', overflow: 'hidden', border: '1.5px solid #e0e0e0', borderLeft: 'none' }}>
                    <button onClick={goToPrev} title="OP précédent" style={{ padding: '3px 8px', background: '#f8f9fa', cursor: 'pointer', border: 'none', borderBottom: '1px solid #e0e0e0', lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="18 15 12 9 6 15"/></svg>
                    </button>
                    <button onClick={goToNext} title="OP suivant" style={{ padding: '3px 8px', background: '#f8f9fa', cursor: 'pointer', border: 'none', lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
                    </button>
                  </div>
                  <span style={{ fontSize: 11, color: '#999', whiteSpace: 'nowrap', marginLeft: 4 }}>{selectedOp ? `${currentIndex + 1}/${opsSource.length}` : `${opsSource.length}`}</span>
                </div>
                {showDropdown && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1.5px solid #e0e0e0', borderTop: 'none', borderRadius: '0 0 8px 8px', maxHeight: 300, overflowY: 'auto', zIndex: 1000, boxShadow: '0 4px 12px rgba(0,0,0,0.1)', minWidth: 360 }}>
                ...
                <>
                  ...
                  <div style={{ flex: '0 1 auto' }}>
                    ...
                    {editNumero !== null ? (
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        <input value={editNumero} onChange={e => setEditNumero(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleSaveNumero(); if (e.key === 'Escape') setEditNumero(null); }} style={{ padding: '6px 8px', border: `1.5px solid ${accent}40`, borderRadius: 6, fontFamily: 'monospace', fontWeight: 700, fontSize: 12, background: '#fffde7', width: 200 }} autoFocus />
                        <button onClick={handleSaveNumero} style={{ border: 'none', background: '#1b5e20', color: '#fff', borderRadius: 4, padding: '6px 8px', fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>✓</button>
                        <button onClick={() => setEditNumero(null)} style={{ border: 'none', background: '#999', color: '#fff', borderRadius: 4, padding: '6px 8px', fontSize: 11, cursor: 'pointer' }}>✕</button>
                      </div>
                    ) : (
                      <span style={{ padding: '6px 8px', background: '#f8f9fa', border: '1.5px solid #e0e0e0', borderRadius: 6, fontFamily: 'monospace', fontWeight: 800, fontSize: 12, display: 'inline-flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
                        {selectedOp.numero} <span onClick={handleStartEditNumero} style={{ opacity: 0.3, cursor: 'pointer', fontSize: 11 }}>✏️</span>
                      </span>
                    )}
                  </div>
                  ...
                  <div style={{ flex: '0 0 auto' }}>
                    ...
                    {isEditMode ? (
                      ...
                      <span style={{ padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 700, background: (typeColors[selectedOp.type] || '#999') + '15', color: typeColors[selectedOp.type] || '#999', display: 'inline-block' }}>{selectedOp.type}</span>
                    )}
                  </div>
                  ...
                  <div style={{ flex: '0 0 auto' }}>
                    ...
                    <span style={{ padding: '6px 8px', background: '#f8f9fa', border: '1.5px solid #e0e0e0', borderRadius: 6, fontFamily: 'monospace', fontSize: 11, display: 'inline-block', whiteSpace: 'nowrap' }}>{selectedOp.dateCreation || ''}</span>
                  </div>

                  {/* OP PROVISOIRE — inline (ANNULATION/DEFINITIF) */}
                  {['ANNULATION', 'DEFINITIF'].includes(isEditMode ? form.type : selectedOp.type) && (
                    <div style={{ flex: '0 0 auto' }}>
                      <label style={{ display: 'block', fontSize: 9, fontWeight: 700, marginBottom: 3, color: (isEditMode ? form.type : selectedOp.type) === 'ANNULATION' ? '#c62828' : '#2e7d32' }}>🔄 OP PROV.</label>
                      {isEditMode ? (
                        <Autocomplete
                        ...
                        <span style={{ padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, fontFamily: 'monospace', display: 'inline-block', background: selectedOp.type === 'ANNULATION' ? '#ffebee' : '#e8f5e9', border: `1px solid ${selectedOp.type === 'ANNULATION' ? '#ffcdd2' : '#c8e6c9'}` }}>
                  ...
                  {/* STATUT — toujours en dernier */}
                  {statutInfo && (
                    <div style={{ flex: '0 0 auto', marginLeft: 'auto' }}>
                      <label style={labelStyle}>STATUT</label>
                      <span style={{ padding: '5px 10px', borderRadius: 6, background: statutInfo.bg, color: statutInfo.color, fontWeight: 700, fontSize: 10, display: 'inline-block', whiteSpace: 'nowrap' }}>{statutInfo.icon} {statutInfo.label}</span>
                    </div>
                  )}
                </>
              )}
              ...
              <>
                ...
                <div style={isReadOnly ? { pointerEvents: 'none' } : {}}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 3fr', gap: 16, alignItems: 'end' }}>
                      ...
                      <div style={{ display: 'flex', gap: 10, alignItems: 'end' }}>
                        <div>
                          ...
                          <div style={{ ...fieldStyle, height: 38, display: 'flex', alignItems: 'center', whiteSpace: 'nowrap' }}>{selectedBeneficiaire?.ncc || ''}</div>
                        </div>
                        <div>
                          ...
                          <div style={{ display: 'flex', gap: 4 }}>
                            {['ESPECES', 'CHEQUE', 'VIREMENT'].map(mode => {
                              ...
                              return (
                                <div key={mode} onClick={() => isEditMode && setForm({ ...form, modeReglement: mode })} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 8px', borderRadius: 6, border: `1.5px solid ${active ? accent : '#e0e0e0'}`, background: active ? accent + '08' : 'white', cursor: isEditMode ? 'pointer' : 'default' }}>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    ...
                    {/* BUDGET (col 1+2) + TVA+REJET (col 3) */}
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 3fr', gap: 16 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                          ...
                          <div>
                            ...
                            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                              {[{ val: true, lbl: 'OUI' }, { val: false, lbl: 'NON' }].map(opt => {
                                ...
                                return (
                                  <div key={opt.lbl} onClick={() => isEditMode && setForm({ ...form, tvaRecuperable: opt.val })} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 6, border: `1.5px solid ${active ? accent : '#e0e0e0'}`, background: active ? accent + '08' : 'white', cursor: isEditMode ? 'pointer' : 'default' }}>
                                    <div style={{ width: 10, height: 10, borderRadius: '50%', border: `2px solid ${active ? accent : '#ccc'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                      {active && <div style={{ width: 4, height: 4, borderRadius: '50%', background: accent }} />}
                                    </div>
                        ...
                        {/* Motif rejet — sous libellé, flex:1 pour équilibrer */}
                        {selectedOp.motifRejet && (
                          <div style={{ padding: 12, background: '#ffebee', borderRadius: 8, border: '1.5px solid #ffcdd2', flex: 1 }}>
                            <div style={{ fontSize: 10, fontWeight: 700, color: '#c62828', marginBottom: 4 }}>❌ MOTIF DU REJET</div>
                            <div style={{ fontSize: 12, color: '#c62828', lineHeight: 1.4 }}>{selectedOp.motifRejet}</div>
                          </div>
                        )}
                      </div>
                    </div>
