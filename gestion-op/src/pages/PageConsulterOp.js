...
const PageConsulterOp = () => {
  ...
  const handleSupprimer = async () => {
    ...
    if (pwd !== (projet?.motDePasseAdmin || 'admin123')) {
      if (pwd !== null) showToast('error', 'Mot de passe incorrect');
      return;
    }
    const opsSuivants = ops.filter(o => o.sourceId === selectedOp.sourceId && o.exerciceId === selectedOp.exerciceId && o.ligneBudgetaire === selectedOp.ligneBudgetaire && (o.createdAt || '') > (selectedOp.createdAt || '') && o.id !== selectedOp.id);
  ...
  return (
    <div>
      ...
      {!exerciceActif ? (
        <div style={{ maxWidth: 900, margin: '0 auto', background: 'white', borderRadius: 12, textAlign: 'center', padding: 40 }}>
                ...
                <div style={isReadOnly ? { pointerEvents: 'none' } : {}}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 160px', gap: 16 }}>
                      <div>
                        ...
                        {isEditMode ? (
                          ...
                          <div style={fieldStyle}><span style={{ fontWeight: 600 }}>{selectedBeneficiaire?.nom || 'N/A'}</span></div>
                        )}
                      </div>
                      <div>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      ...
                      {isEditMode ? <textarea value={form.objet} onChange={(e) => setForm({ ...form, objet: e.target.value })} style={{ ...editFieldStyle, width: '100%', minHeight: 70, resize: 'vertical', fontFamily: 'inherit', fontSize: 13, outline: 'none' }} />
                      : <div style={{ ...fieldStyle, minHeight: 44 }}>{form.objet || ''}</div>}
                    </div>
                    {(form.piecesJustificatives || isEditMode) && (
                      <div>
                        <label style={labelStyle}>PIÈCES JUSTIFICATIVES</label>
                        {isEditMode ? <textarea value={form.piecesJustificatives} onChange={(e) => setForm({ ...form, piecesJustificatives: e.target.value })} style={{ ...editFieldStyle, width: '100%', minHeight: 50, resize: 'vertical', fontFamily: 'inherit', fontSize: 13, outline: 'none' }} />
                        : <div style={{ ...fieldStyle, fontSize: 12, color: '#555' }}>{form.piecesJustificatives || ''}</div>}
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '200px 180px 1fr', gap: 16, alignItems: 'start' }}>
                      <div>
                        ...
                        {isEditMode ? <MontantInput value={form.montant} onChange={(val) => setForm({ ...form, montant: val })} style={{ ...editFieldStyle, fontFamily: 'monospace', fontSize: 16, textAlign: 'right', width: '100%' }} placeholder="0" />
                      ...
                      <div>
                        ...
                        {isEditMode ? <Autocomplete options={(currentBudget?.lignes || []).map(l => ({ value: l.code, label: `${l.code} - ${l.libelle}`, searchFields: [l.code, l.libelle] }))} value={form.ligneBudgetaire ? (currentBudget?.lignes || []).filter(x => x.code === form.ligneBudgetaire).map(l => ({ value: l.code, label: `${l.code} - ${l.libelle}` }))[0] || null : null} onChange={(option) => setForm({ ...form, ligneBudgetaire: option?.value || '' })} placeholder="🔍 Code..." accentColor={accent} />
                      ...
                      <div style={{ gridColumn: '1 / 3' }}>
                        <div style={{ background: '#f8faf9', padding: 14, borderRadius: 10, border: '1px solid #e8ece9' }}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 150px', gap: 5 }}>
                            ...
                            <span style={{ fontSize: 14, fontFamily: 'monospace', textAlign: 'right', fontWeight: 800, color: (getDotation() - getEngagementsAnterieurs()) >= 0 ? '#2e7d32' : '#c62828' }}>{formatMontant(getDotation() - getEngagementsAnterieurs())}</span>
                          </div>
                        </div>
                      </div>
                      <div>
                          ...
                          <>
                            {isEditMode ? (
                              <>
                                <div style={{ marginBottom: 12 }}>
                                  ...
                                  <div style={{ display: 'flex', gap: 16 }}>
                                    {[{ val: true, lbl: 'OUI' }, { val: false, lbl: 'NON' }].map(opt => {
                                      ...
                                      return (
                                        <div key={opt.lbl} onClick={() => setForm({ ...form, tvaRecuperable: opt.val })} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, border: `1.5px solid ${active ? accent : '#e0e0e0'}`, background: active ? accent + '08' : 'white', cursor: 'pointer' }}>
                                ...
                                {form.tvaRecuperable && <div><label style={labelStyle}>MONTANT TVA</label><MontantInput value={form.montantTVA} onChange={(val) => setForm({ ...form, montantTVA: val })} style={{ ...editFieldStyle, fontFamily: 'monospace', textAlign: 'right', width: '100%' }} placeholder="0" /></div>}
                              </>
                            ) : (
                              <div>
                                <label style={labelStyle}>TVA RÉCUPÉRABLE</label>
