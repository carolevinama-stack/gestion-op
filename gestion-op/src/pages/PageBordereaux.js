...
const PageBordereaux=()=>{
  ...
  const closeAllModals=()=>{setModalRetourCF(false);setModalPaiement(null);setModalArchive(false);setModalEditBT(null);setModalSuppressionHist(false);};

  const genNumeroBT=async(typeBT, specificSourceId)=>{
    ...
    const cId=`${typeBT}_${specificSourceId}_${exerciceActif?.id}`;
    const cRef=doc(db,'compteurs',cId);
    return await runTransaction(db,async(tx)=>{
      const snap=await tx.get(cRef);const next=(snap.exists()?(snap.data().count||0):0)+1;
      tx.set(cRef,{count:next,type:typeBT,sourceId:specificSourceId,exerciceId:exerciceActif?.id});
  ...
  const handlePrintBordereau=(bt)=>{
    ...
    const rows=btOps.map((op,i)=>`<tr><td style="text-align:center">${i+1}</td><td>${escapeHtml(getBen(op))}</td><td>${escapeHtml(op.objet||'-')}</td><td style="text-align:center;font-family:monospace;font-size:9px;font-weight:bold">${escapeHtml(op.numero)}</td><td style="text-align:center">${nbEx}</td><td style="text-align:right;font-family:monospace;font-weight:bold">${Number(op.montant||0).toLocaleString('fr-FR')}</td></tr>`).join('');
