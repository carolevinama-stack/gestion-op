  ...
  return<div>
    ...
    {modalArchive&&selectedOps.length>0&&<Modal title={`Archivage — ${selectedOps.length} OP`} titleColor={P.olive} onClose={()=>setModalArchive(false)}>
      ...
      <div style={{display:'flex', justifyContent:'flex-end', gap:12}}>
         ...
         <button onClick={handleArchiver} disabled={saving||!boiteArchivage.trim()} style={{padding:'10px 24px',border:'none',borderRadius:8,background:P.olive,color:'white',fontWeight:700,cursor:'pointer',opacity:!boiteArchivage.trim()?.5:1}}>{saving?'Archivage...':`Archiver`}</button>
      </div>
    </Modal>}
