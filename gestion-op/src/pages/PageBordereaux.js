        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={() => handlePrintBordereau(bt)} title="Imprimer"
              style={{ background: '#e3f2fd', color: '#1565c0', border: 'none', borderRadius: 6, padding: '6px 12px', cursor: 'pointer', fontSize: 13 }}>
              🖨️
            </button>
            {isPrep && (
              <>
                <button onClick={() => setShowAddOps(showAddOps === bt.id ? null : bt.id)} title="Ajouter un OP"
                  style={{ background: '#e8f5e9', color: '#2e7d32', border: 'none', borderRadius: 6, padding: '6px 12px', cursor: 'pointer', fontSize: 13 }}>
                  ➕
                </button>
        ...
        {[
          ...
          <button key={tab.key} onClick={() => changeTab(tab.key)}
            style={{
              ...
              borderRadius: tab.key === 'CF' ? '8px 0 0 8px' : tab.key === 'ARCHIVES' ? '0 8px 8px 0' : 0,
              transition: 'all 0.2s'
            }}>
            {tab.label}
          </button>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        {[
                          ...
                          <button key={opt.val} onClick={() => setResultatCF(opt.val)}
                            style={{
                              flex: 1, padding: '10px 6px', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer',
