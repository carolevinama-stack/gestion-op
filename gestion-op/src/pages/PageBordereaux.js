...
const PageBordereaux = () => {
  ...
  const opsEligiblesCF = opsForSource.filter(op => op.statut === 'CREE' || op.statut === 'DIFFERE_CF');
  // CF - Transmis au CF (en attente retour)
  const opsTransmisCF = opsForSource.filter(op => op.statut === 'TRANSMIS_CF');
  // CF - Différés à satisfaire
  ...
  // ================================================================
  // ACTION : RÉINTRODUIRE OP DIFFÉRÉ (retour dans Nouveau BT)
  // ================================================================
  const handleReintroduire = async (opIds, dateReintroduction) => {
    if (!dateReintroduction) { alert('Veuillez saisir la date de réintroduction.'); return; }
    if (!window.confirm(`Réintroduire ${opIds.length} OP différé(s) dans le circuit ?\n\nDate : ${dateReintroduction}\nIls apparaîtront dans "Nouveau BT au CF".`)) return;

    setSaving(true);
    try {
      for (const opId of opIds) {
        const op = ops.find(o => o.id === opId);
        // Sauvegarder l'historique des différés
        const historiqueDifferes = op?.historiqueDifferes || [];
        historiqueDifferes.push({
          dateDiffere: op?.dateDiffere,
          motifDiffere: op?.motifDiffere,
          dateReintroduction,
      ...
      alert(`✅ ${opIds.length} OP réintroduit(s). Ils sont disponibles dans "Nouveau BT au CF".`);
      setSelectedOps([]);
    } catch (error) {
      alert('Erreur : ' + error.message);
    }
...
<div class="accuse">
  ...
  const BordereauCard = ({ bt }) => {
    ...
    const availableOps = bt.type === 'CF'
      ? opsForSource.filter(op => (op.statut === 'CREE' || op.statut === 'DIFFERE_CF') && !bt.opsIds.includes(op.id))
      ...
      {mainTab === 'CF' && (
        <div>
          <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
            ...
            <SubTabBtn active={subTabCF === 'TRANSMIS'} label="📤 OP Transmis" count={opsTransmisCF.length + opsDifferesCF.length + opsRejetesCF.length} color="#e65100" onClick={() => changeSubTab(setSubTabCF, 'TRANSMIS')} />
            ...
            <div style={styles.card}>
              ...
              <OPTable opsList={opsEligiblesCF} extraColumns={[
                { key: 'statut', label: 'STATUT', width: 80, render: (op) => (
                  <Badge bg={op.statut === 'DIFFERE_CF' ? '#fff3e0' : '#e8f5e9'} color={op.statut === 'DIFFERE_CF' ? '#e65100' : '#2e7d32'}>
                    {op.statut === 'DIFFERE_CF' ? 'Différé' : 'Créé'}
                  </Badge>
                )}
              ]} />
          ...
          {subTabCF === 'TRANSMIS' && (
            <div>
              {/* ========== ZONE 1 : EN ATTENTE DE RETOUR ========== */}
              <div style={styles.card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <h3 style={{ margin: 0, color: '#e65100' }}>🔶 En attente de retour du CF ({opsTransmisCF.length})</h3>
                </div>

                {opsTransmisCF.length === 0 ? (
                  <EmptyState text="Aucun OP en attente de retour" />
                ) : (
                  <>
                    ...
                    {selectedOps.length > 0 && selectedOps.some(id => opsTransmisCF.find(op => op.id === id)) && (
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                              {[
                                ...
                                <button key={opt.val} onClick={() => setResultatCF(opt.val)}
                                  style={{
                                    flex: 1, padding: '10px 8px', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer',
              ...
              {/* ========== ZONE 2 : DIFFÉRÉS À SATISFAIRE ========== */}
              {opsDifferesCF.length > 0 && (
                <div style={{ ...styles.card, marginTop: 16, border: '2px solid #ff9800' }}>
                  <h3 style={{ margin: '0 0 4px', color: '#e65100' }}>⏳ Différés à satisfaire ({opsDifferesCF.length})</h3>
                  <p style={{ fontSize: 12, color: '#888', marginBottom: 16 }}>
                    OP différés par le CF. Sélectionnez et réintroduisez quand le dossier est corrigé.
                  </p>
                    ...
                    <table style={styles.table}>
                      <thead>
                        <tr>
                          <th style={{ ...styles.th, width: 36 }}>
                            <input type="checkbox"
                              checked={selectedOps.length === opsDifferesCF.length && opsDifferesCF.length > 0}
                              onChange={() => {
                                if (selectedOps.length === opsDifferesCF.length) setSelectedOps([]);
                                else setSelectedOps(opsDifferesCF.map(op => op.id));
                              }} />
                          </th>
                          <th style={{ ...styles.th, width: 130 }}>N° OP</th>
                          <th style={styles.th}>BÉNÉFICIAIRE</th>
                          <th style={{ ...styles.th, width: 110, textAlign: 'right' }}>MONTANT</th>
                          <th style={{ ...styles.th, width: 100 }}>DATE DIFFÉRÉ</th>
                          <th style={styles.th}>MOTIF</th>
                          <th style={{ ...styles.th, width: 50, textAlign: 'center' }}>NB</th>
                        </tr>
                      </thead>
                      <tbody>
                        {opsDifferesCF.map(op => {
                          const ben = beneficiaires.find(b => b.id === op.beneficiaireId);
                          const checked = selectedOps.includes(op.id);
                          const nbDifferes = (op.historiqueDifferes || []).length + 1;
                          return (
                            <tr key={op.id} onClick={() => toggleOp(op.id)}
                              style={{ cursor: 'pointer', background: checked ? '#fff3e0' : 'transparent' }}>
                              <td style={styles.td}><input type="checkbox" checked={checked} onChange={() => toggleOp(op.id)} /></td>
                              <td style={{ ...styles.td, fontFamily: 'monospace', fontWeight: 600, fontSize: 10 }}>{op.numero}</td>
                              <td style={{ ...styles.td, fontSize: 12 }}>{ben?.nom || 'N/A'}</td>
                              <td style={{ ...styles.td, textAlign: 'right', fontFamily: 'monospace', fontWeight: 600 }}>{formatMontant(op.montant)}</td>
                              <td style={{ ...styles.td, fontSize: 12 }}>{op.dateDiffere || '-'}</td>
                              <td style={{ ...styles.td, fontSize: 11, maxWidth: 200 }}>{op.motifDiffere || '-'}</td>
                              <td style={{ ...styles.td, textAlign: 'center' }}>
                                {nbDifferes > 1 && (
                                  <Badge bg="#fff3e0" color="#e65100">{nbDifferes}x</Badge>
                                )}
                              </td>
                            </tr>
                          );
                  ...
                  {selectedOps.length > 0 && selectedOps.some(id => opsDifferesCF.find(op => op.id === id)) && (
                    <div style={{ marginTop: 16, padding: 16, background: '#fff8e1', borderRadius: 8, border: '1px solid #ffcc02' }}>
                      <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12 }}>
                        Réintroduire {selectedOps.length} OP dans le circuit
                      </div>
                      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                        <div style={{ minWidth: 200 }}>
                          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Date de réintroduction</label>
                          <input type="date" value={dateReintroduction} onChange={e => setDateReintroduction(e.target.value)}
                            style={{ ...styles.input, marginBottom: 0 }} />
                        </div>
                        <button onClick={() => handleReintroduire(selectedOps, dateReintroduction)} disabled={saving}
                          style={{ ...styles.button, padding: '12px 30px', fontSize: 14, background: '#e65100', marginBottom: 0 }}>
                          {saving ? '...' : `🔄 Réintroduire (${selectedOps.length} OP)`}
                        </button>
                      </div>
                    </div>
                  )}
              ...
              {/* ========== ZONE 3 : OP REJETÉS ========== */}
              {opsRejetesCF.length > 0 && (
                <div style={{ ...styles.card, marginTop: 16, border: '2px solid #ef5350' }}>
                  <h3 style={{ margin: '0 0 4px', color: '#c62828' }}>❌ OP Rejetés ({opsRejetesCF.length})</h3>
                  <p style={{ fontSize: 12, color: '#888', marginBottom: 16 }}>
                    Budget automatiquement libéré pour ces OP.
                  </p>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={styles.table}>
                      <thead>
                        <tr>
                          ...
                          <th style={styles.th}>OBJET</th>
                          <th style={{ ...styles.th, width: 110, textAlign: 'right' }}>MONTANT</th>
                          <th style={{ ...styles.th, width: 100 }}>DATE REJET</th>
                          <th style={styles.th}>MOTIF</th>
                        </tr>
                      </thead>
                      <tbody>
                        {opsRejetesCF.map(op => {
                          const ben = beneficiaires.find(b => b.id === op.beneficiaireId);
                          return (
                            <tr key={op.id} style={{ background: '#fff5f5' }}>
                              <td style={{ ...styles.td, fontFamily: 'monospace', fontWeight: 600, fontSize: 10 }}>{op.numero}</td>
                              <td style={{ ...styles.td, fontSize: 12 }}>{ben?.nom || 'N/A'}</td>
                              <td style={{ ...styles.td, fontSize: 11, maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{op.objet || '-'}</td>
                              <td style={{ ...styles.td, textAlign: 'right', fontFamily: 'monospace', fontWeight: 600, color: '#c62828' }}>{formatMontant(op.montant)}</td>
                              <td style={{ ...styles.td, fontSize: 12 }}>{op.dateRejet || '-'}</td>
                              <td style={{ ...styles.td, fontSize: 11, maxWidth: 200 }}>{op.motifRejet || '-'}</td>
                            </tr>
                          );
                        })}
                      </tbody>
