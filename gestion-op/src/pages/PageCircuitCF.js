...
const I={
  ...
  refresh:(c=P.textSec,s=14)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 4v6h-6"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg>,
};
...
const PageCircuitCF = () => {
  ...
  // SCRIPT DE RÉPARATION DES OP ORPHELINS
  const handleFixOrphanOps = async () => {
    ask("Réparation", "Vider le champ bordereau des OP dont le bordereau n'existe plus ?", async () => {
      setSaving(true);
      try {
        const batch = writeBatch(db);
        let fixedCount = 0;
        // On cherche les OP qui ont un bordereauCF renseigné mais qui n'apparaissent pas dans les bordereaux actifs
        ops.forEach(op => {
        ...
        if (fixedCount > 0) {
          await batch.commit();
          notify("success", "Réparé", `${fixedCount} OP orphelins ont été libérés.`);
        } else {
          notify("success", "Infos", "Aucun OP orphelin détecté.");
        }
      } catch(e) {
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
      ...
      <button onClick={handleFixOrphanOps} style={{padding:'8px 12px',background:P.goldLight,border:`1px solid ${P.goldBorder}`,borderRadius:8,display:'flex',alignItems:'center',gap:8,cursor:'pointer',fontSize:11,fontWeight:700,color:P.gold,boxShadow:'0 1px 2px rgba(0,0,0,.05)'}}>
        {I.refresh(P.gold,14)} Réparer OP Orphelins
      </button>
    </div>
