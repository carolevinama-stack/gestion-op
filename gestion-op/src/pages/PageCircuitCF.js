...
const handlePrintBordereau = (bt) => {
  ...
  // 1. Récupérer la valeur par défaut des paramètres (4 si non défini)
  const nbExParametre = projet?.nbExemplairesCF || 4;

  // 2. Génération des lignes avec logique conditionnelle pour le nombre d'exemplaires
  const rows = btOps.map((op, i) => {
    // Logique : Si c'est une annulation, on force à 2, sinon on prend le paramètre
    const nbExemplairesAFicher = op.type === 'ANNULATION' ? 2 : nbExParametre;

    // Construction de la ligne HTML (Bénéficiaire maintenu à 150px)
    return `<tr>
      <td style="text-align:center">${i + 1}</td>
      <td style="width:150px">${escapeHtml(getBen(op))}</td>
      <td>${escapeHtml(op.objet || '-')}</td>
      <td style="text-align:center;font-family:monospace;font-size:9px;font-weight:bold">${escapeHtml(op.numero)}</td>
      <td style="text-align:center">${nbExemplairesAFicher}</td>
