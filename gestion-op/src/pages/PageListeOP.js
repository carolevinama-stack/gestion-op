...
const PageConsulterOp = () => {
  ...
  // Cumul chronologique simple : somme des OP précédents, même source, même ligne
  ...
  const handlePrint = () => {
    ...
    const engCumules = engAnterieurs + montantOP;
    const disponible = getDotation() - engCumules;
    const fmtSigne = (val) => val < 0 ? ('-' + formatMontant(Math.abs(val))) : formatMontant(val);
    const printMontantTotal = fmtSigne(montantOP);
    const printEngActuel = fmtSigne(montantOP);
  ...
  return (
    <div className="consulterop-form" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* Modal (mot de passe / confirmation) */}
      {modal && (
                        ...
                        return (
                          <div key={op.id} onClick={() => loadOp(op)} style={{ padding: '10px 14px', borderBottom: `1px solid ${P.bgApp}`, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: isSelected ? accent + '0a' : 'transparent', transition: 'background 0.15s' }}
                            ...
                            <div style={{ textAlign: 'right' }}>
                              <div style={{ fontWeight: 700, fontFamily: 'monospace', color: (op.montant || 0) < 0 ? '#C43E3E' : accent, fontSize: 12 }}>{(op.montant || 0) < 0 ? '-' + formatMontant(Math.abs(op.montant)) : formatMontant(op.montant)} F</div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 3fr', gap: 16, marginBottom: 16 }}>
                        <div>
                          ...
                          : <div style={{ ...fieldStyle, fontFamily: 'monospace', fontSize: 16, fontWeight: 700, textAlign: 'right', color: (selectedOp.montant || 0) < 0 ? '#C43E3E' : accent }}>{(selectedOp.montant || 0) < 0 ? '-' + formatMontant(Math.abs(selectedOp.montant)) : formatMontant(selectedOp.montant)}</div>}
                        </div>
