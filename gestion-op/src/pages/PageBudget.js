...
const PageBudget = () => {
  ...
  const createRevision = async () => {
    if (!nomRevision.trim()) {
      alert('Veuillez nommer cette révision');
      return;
    }
    if (!dateNotification) {
      alert('Veuillez renseigner la date de validation');
      return;
    }
  ...
  const handleSave = async () => {
    ...
    // Bloquer si des lignes ont une dotation inférieure aux engagements
    ...
    if (lignesDepassees.length > 0) {
      ...
      alert(`❌ Impossible de valider\n\nLa dotation de ${lignesDepassees.length} ligne(s) est inférieure aux engagements :\n\n${detail}\n\nCorrigez les dotations avant d'enregistrer.`);
      return;
    }
              ...
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #e9ecef', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                {allBudgetsForSourceExercice.length > 1 && (
                  <button onClick={() => {
                    ...
                    📜 Historique ({allBudgetsForSourceExercice.length} révisions)
                  </button>
                )}
              </div>
            )}
          ...
          <div style={{ ...styles.modalContent, maxWidth: 800 }}>
            ...
            <div style={{ padding: 24 }}>
              ...
              {!currentBudget && <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Nom de la révision *</label>
                ...
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Date de validation *</label>
      ...
      {showRevisionModal && (
        <div style={styles.modal}>
          <div style={{ ...styles.modalContent, maxWidth: 500 }}>
            <div style={{ padding: 24, borderBottom: '1px solid #e9ecef', background: currentSourceObj?.couleur || '#0f4c3a', color: 'white' }}>
              <h2 style={{ margin: 0, fontSize: 18 }}>📝 Nouvelle révision budgétaire</h2>
            </div>
            <div style={{ padding: 24 }}>
