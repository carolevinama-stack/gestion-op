...
const PageBudget = () => {
  ...
  const createRevision = async () => {
    if (!nomRevision.trim()) {
      alert('Veuillez nommer cette notification');
      return;
    }
    if (!dateNotification) {
      alert('Veuillez renseigner la date de notification');
      return;
    }
  ...
  // Modifier la dotation d'une ligne (pas de validation pendant la saisie)
  ...
  const handleSave = async () => {
    ...
    // Vérifier les lignes dont la dotation est inférieure aux engagements
    const lignesDepassees = budgetLignes.filter(l => {
      const eng = getEngagementLigne(l.code);
      return eng > 0 && (l.dotation || 0) < eng;
    });
    if (lignesDepassees.length > 0) {
      const detail = lignesDepassees.map(l => `• ${l.code} : dotation ${formatMontant(l.dotation)} < engagements ${formatMontant(getEngagementLigne(l.code))}`).join('\n');
      if (!window.confirm(`⚠️ Attention\n\n${lignesDepassees.length} ligne(s) ont une dotation inférieure aux engagements :\n\n${detail}\n\nCela créera un disponible négatif. Continuer quand même ?`)) return;
    }
            ...
            {currentBudget && (
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #e9ecef', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  {currentBudget.dateNotification && (
                ...
                {allBudgetsForSourceExercice.length > 1 && (
                  <button onClick={() => {
                    ...
                    📜 Historique ({allBudgetsForSourceExercice.length} notifications)
                  </button>
                )}
              </div>
            )}
        ...
        <div style={styles.modal}>
          <div style={{ ...styles.modalContent, maxWidth: 800 }}>
            ...
            <div style={{ padding: 24 }}>
              {/* Champs nom/date pour nouvelle création */}
              {!currentBudget && <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Nom de la notification *</label>
                  <input type="text" value={nomRevision} onChange={e => setNomRevision(e.target.value)} placeholder="Budget Primitif" style={styles.input} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Date de notification *</label>
                  <input type="date" value={dateNotification} onChange={e => setDateNotification(e.target.value)} style={styles.input} />
                </div>
              </div>}
      ...
      {showRevisionModal && (
        <div style={styles.modal}>
          <div style={{ ...styles.modalContent, maxWidth: 500 }}>
            <div style={{ padding: 24, borderBottom: '1px solid #e9ecef', background: currentSourceObj?.couleur || '#0f4c3a', color: 'white' }}>
              <h2 style={{ margin: 0, fontSize: 18 }}>📝 Nouvelle notification budgétaire</h2>
            </div>
            <div style={{ padding: 24 }}>
              <div style={{ background: '#fff3e0', padding: 16, borderRadius: 8, marginBottom: 20 }}>
                ...
                <p style={{ margin: '8px 0 0', fontSize: 13, color: '#555' }}>
                  Une nouvelle version (v{(latestVersion?.version || 1) + 1}) sera créée. 
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input 
                  type="text"
                  value={nomRevision} 
                  onChange={(e) => setNomRevision(e.target.value)}
                  placeholder="Ex: Budget Primitif, Budget Révisé 1, V2..."
                  style={styles.input}
                  autoFocus
                />
              </div>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Motif <span style={{ fontWeight: 400, color: '#999' }}>(optionnel)</span></label>
                <textarea 
                  value={motifRevision} 
            ...
            <div style={{ padding: 24, borderTop: '1px solid #e9ecef', background: '#f8f9fa', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              ...
              <button 
                ...
                disabled={saving || !nomRevision.trim() || !dateNotification}
                style={{ 
                  ...styles.button, 
                  background: currentSourceObj?.couleur || '#0f4c3a',
                  opacity: saving || !nomRevision.trim() || !dateNotification ? 0.6 : 1,
                  cursor: saving || !nomRevision.trim() || !dateNotification ? 'not-allowed' : 'pointer'
                }}
              >
                {saving ? 'Création...' : '✓ Créer la notification'}
              </button>
            </div>
          </div>
        </div>
