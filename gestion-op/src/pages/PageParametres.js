...
const SectionTitle = ({ title, action }) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, paddingBottom: 8, borderBottom: `1px solid ${P.border}` }}>
    <h3 style={{ fontSize: 13, fontWeight: 800, color: P.text, textTransform: 'uppercase', letterSpacing: 0.5, margin: 0 }}>
      {title}
    </h3>
    {action}
  </div>
...
// ============================================================
// ONGLET INFOS (Fusion de Projet, Sources et Exercices)
// ============================================================
const TabInfos = () => {
  const { projet, setProjet, sources, setSources, exercices, setExercices } = useAppContext();
  ...
  const handleDeleteSrc = async (source) => {
    if (!window.confirm(`Supprimer la source "${source.sigle}" ?`)) return;
  ...
  const inputStyle = { ...styles.input, marginBottom: 0, borderRadius: 8, fontSize: 12, padding: '10px 12px' };
  const thStyle = { ...styles.th, fontSize: 10, color: P.textSec, textTransform: 'uppercase', padding: '8px 10px', background: '#FAFAF8' };

  return (
    <div>
      {/* GRILLE DENSE 2 COLONNES */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: 20, alignItems: 'start' }}>
        
        {/* ================= COLONNE GAUCHE (LE PROJET & SES BASES) ================= */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div><Label>Sigle</Label><input value={formProj.sigle || ''} onChange={e => setFormProj({...formProj, sigle: e.target.value})} style={inputStyle} /></div>
            </div>
          </div>

          {/* Sources de financement (Compact) */}
          <div style={{ ...cardStyle, padding: '16px 20px' }}>
            <SectionTitle title="Sources de financement" action={<button onClick={openNewSrc} style={{ background: P.greenLight, color: P.greenDark, border: 'none', borderRadius: 6, padding: '4px 8px', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>{I.plus(P.greenDark, 12)} Ajouter</button>} />
            {sources.length === 0 ? <Empty text="Aucune source" /> : (
              <div style={{ border: `1px solid ${P.border}`, borderRadius: 8, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                  <thead>
                    <tr><th style={thStyle}>Sigle</th><th style={thStyle}>Compte</th><th style={{...thStyle, width: 60}}>Actions</th></tr>
                  </thead>
                  <tbody>
                    {sources.map(s => (
                      <tr key={s.id} style={{ borderBottom: `1px solid ${P.border}` }}>
                        <td style={{ padding: '8px 10px', fontWeight: 700 }}><div style={{display:'flex', alignItems:'center', gap:6}}><div style={{width:8, height:8, borderRadius:'50%', background: s.couleur}}/>{s.sigle}</div></td>
                        <td style={{ padding: '8px 10px' }}><Badge bg={s.compteDebiter==='BAILLEUR'?P.greenLight:P.goldLight} color={s.compteDebiter==='BAILLEUR'?P.greenDark:P.gold}>{s.compteDebiter}</Badge></td>
                        <td style={{ padding: '8px 10px', display: 'flex', gap: 4 }}>
                          <button onClick={() => openEditSrc(s)} style={{ background:'transparent', border:'none', cursor:'pointer', padding:2 }}>{I.edit(P.textSec, 14)}</button>
                          <button onClick={() => handleDeleteSrc(s)} style={{ background:'transparent', border:'none', cursor:'pointer', padding:2 }}>{I.trash(P.red, 14)}</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {/* Exercices budgétaires (Compact) */}
          <div style={{ ...cardStyle, padding: '16px 20px' }}>
            <SectionTitle title="Exercices budgétaires" action={<button onClick={() => { setFormEx({ annee: new Date().getFullYear() + 1, actif: false }); setShowExModal(true); }} style={{ background: P.greenLight, color: P.greenDark, border: 'none', borderRadius: 6, padding: '4px 8px', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>{I.plus(P.greenDark, 12)} Ajouter</button>} />
            {exercices.length === 0 ? <Empty text="Aucun exercice" /> : (
              <div style={{ border: `1px solid ${P.border}`, borderRadius: 8, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                  <thead>
                    <tr><th style={thStyle}>Année</th><th style={thStyle}>Statut</th><th style={{...thStyle, width: 80, textAlign:'right'}}>Action</th></tr>
                  </thead>
                  <tbody>
                    {exercices.map(ex => (
                      <tr key={ex.id} style={{ borderBottom: `1px solid ${P.border}`, background: ex.actif ? P.greenLight : 'transparent' }}>
                        <td style={{ padding: '8px 10px', fontWeight: 800, fontSize: 13, color: ex.actif ? P.greenDark : P.text }}>{ex.annee}</td>
                        <td style={{ padding: '8px 10px' }}>{ex.actif ? <Badge bg={P.greenDark} color="#fff">Actif</Badge> : <span style={{color:P.textMuted}}>Clôturé</span>}</td>
                        <td style={{ padding: '8px 10px', textAlign: 'right' }}>
                          {!ex.actif && <button onClick={() => setActifEx(ex)} style={{ background: '#fff', border: `1px solid ${P.border}`, padding: '4px 8px', borderRadius: 4, cursor: 'pointer', fontSize: 10, fontWeight: 600 }}>Activer</button>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {/* ================= COLONNE DROITE (TECHNIQUE & SÉCURITÉ) ================= */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ ...cardStyle, padding: '16px 20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div><Label>Nom / Prénom</Label><input value={formProj.coordonnateur || ''} onChange={e => setFormProj({...formProj, coordonnateur: e.target.value})} style={inputStyle} /></div>
              <div><Label>Titre (Bordereaux)</Label><input value={formProj.titreCoordonnateur || ''} onChange={e => setFormProj({...formProj, titreCoordonnateur: e.target.value})} style={inputStyle} /></div>
            </div>
          </div>

          {/* Config Technique */}
          <div style={{ ...cardStyle, padding: '16px 20px' }}>
            <SectionTitle title="Normes d'imputation" />
          ...
          {/* Sécurité */}
          <div style={{ ...cardStyle, padding: '16px 20px', background: P.goldLight, border: `1px solid ${P.goldBorder}` }}>
            <SectionTitle title="Sécurité d'administration" />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <Label>Nouveau mot de passe</Label>
                <div style={{ position: 'relative' }}>
                  <input type={showPassword ? 'text' : 'password'} value={formProj.adminPassword || ''} onChange={e => setFormProj({...formProj, adminPassword: e.target.value})} style={{ ...inputStyle, paddingRight: 35 }} />
            ...
            {formProj.adminPassword && confirmPassword && formProj.adminPassword !== confirmPassword && <div style={{ color: P.red, fontSize: 10, fontWeight: 700, marginBottom: 6 }}>Les mots de passe diffèrent</div>}
            <div style={{ fontSize: 11, color: P.textSec }}>Requis pour : purges, suppressions, modifications critiques.</div>
          </div>

          {/* Bouton de sauvegarde global */}
          <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 12, marginTop: 10 }}>
            {savedProj && <span style={{ color: P.green, fontWeight: 700, fontSize: 12 }}>✓ Enregistré</span>}
            <ActionBtn label={savingProj ? "Enregistrement..." : "Sauvegarder les paramètres"} icon={I.check()} color={P.greenDark} onClick={handleSaveProjet} disabled={savingProj} />
          </div>

        </div>
      </div>

      {/* ================= MODALES POUR SOURCES ET EXERCICES ================= */}
      {showSrcModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', backdropFilter: 'blur(3px)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 12, width: 450, boxShadow: '0 20px 60px rgba(0,0,0,.2)', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', background: P.greenDark, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ color: '#fff', margin: 0, fontSize: 14 }}>{editSource ? 'Modifier la source' : 'Nouvelle source'}</h3>
              <button onClick={() => setShowSrcModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>{I.close('#fff')}</button>
            </div>
            <div style={{ padding: 20 }}>
              ...
              <div style={{ marginBottom: 12 }}><Label>Description</Label><textarea value={formSrc.description || ''} onChange={e => setFormSrc({...formSrc, description: e.target.value})} style={{ ...inputStyle, minHeight: 60, resize: 'vertical' }} /></div>
              <div><Label>Couleur</Label><input type="color" value={formSrc.couleur} onChange={e => setFormSrc({...formSrc, couleur: e.target.value})} style={{ width: 60, height: 30, border: 'none', borderRadius: 6, cursor: 'pointer', padding: 0 }} /></div>
            </div>
            <div style={{ padding: '12px 20px', background: '#FAFAF8', borderTop: `1px solid ${P.border}`, display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button onClick={() => setShowSrcModal(false)} style={{ padding: '8px 16px', borderRadius: 8, border: `1px solid ${P.border}`, background: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: 12 }}>Annuler</button>
      ...
      {showExModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', backdropFilter: 'blur(3px)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 12, width: 350, boxShadow: '0 20px 60px rgba(0,0,0,.2)', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', background: P.greenDark, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ color: '#fff', margin: 0, fontSize: 14 }}>Nouvel exercice</h3>
              <button onClick={() => setShowExModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>{I.close('#fff')}</button>
            </div>
            <div style={{ padding: 20 }}>
              <div style={{ marginBottom: 16 }}><Label>Année</Label><input type="number" value={formEx.annee} onChange={e => setFormEx({...formEx, annee: parseInt(e.target.value)})} style={inputStyle} /></div>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', background: P.bg, padding: '10px 12px', borderRadius: 8 }}>
                <input type="checkbox" checked={formEx.actif} onChange={e => setFormEx({...formEx, actif: e.target.checked})} />
                <span style={{ fontSize: 12, fontWeight: 600 }}>Définir comme exercice actif</span>
              </label>
            </div>
            <div style={{ padding: '12px 20px', background: '#FAFAF8', borderTop: `1px solid ${P.border}`, display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button onClick={() => setShowExModal(false)} style={{ padding: '8px 16px', borderRadius: 8, border: `1px solid ${P.border}`, background: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: 12 }}>Annuler</button>
...
const TabMaintenance = () => {
  ...
  const handleRecalerCompteurs = async () => {
    ...
    try {
      ...
      showMsg(`${fixed} compteurs recalés.`);
    } catch (e) { alert('Erreur : ' + e.message); }
    setSaving(false);
  };
  ...
  const cardStyle = { background: 'white', borderRadius: 12, padding: 20, marginBottom: 16, border: `1px solid ${P.border}` };
  
  const btnTool = (id, iconSvg, label, desc, color) => (
    <div key={id} onClick={() => setTool(tool === id ? null : id)}
      style={{ flex: 1, minWidth: 200, padding: 16, borderRadius: 12, border: tool === id ? `2px solid ${color}` : `1px solid ${P.border}`, background: tool === id ? `${color}10` : 'white', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ width: 36, height: 36, borderRadius: 10, background: `${color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{iconSvg}</div>
  ...
  return (
    <div>
      ...
      {message && <div style={{ padding: 12, borderRadius: 8, marginBottom: 16, fontSize: 12, fontWeight: 700, background: message.type === 'success' ? P.greenLight : P.redLight, color: message.type === 'success' ? P.greenDark : P.red }}>{message.text}</div>}

      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        {btnTool('purge', I.trash(P.red, 18), 'Purge des données', 'Supprimer OP et bordereaux.', P.red)}
        {btnTool('import', I.project('#3B6B8A', 18), 'Import Excel', 'Importer massivement des OP.', '#3B6B8A')}
        {btnTool('compteurs', I.calendar(P.gold, 18), 'Recaler compteurs', 'Corriger la numérotation.', P.gold)}
        {btnTool('doublons', I.source(P.gold, 18), 'Doublons', 'Corriger les N° identiques.', P.gold)}
      </div>

      {tool === 'purge' && (
        <div style={cardStyle}>
          <h3 style={{ color: P.red, margin: '0 0 16px', fontSize: 14, fontWeight: 700 }}>Purge des données</h3>
          ...
          <div style={{ marginTop: 16 }}><Label>Tapez SUPPRIMER pour confirmer :</Label><input value={purgeConfirm} onChange={e => setPurgeConfirm(e.target.value)} style={{ ...styles.input, width: 200, marginTop: 4, fontFamily: 'monospace', fontWeight: 700, marginBottom: 0 }} /></div>
          <div style={{ marginTop: 20 }}><ActionBtn label={saving ? "Suppression en cours..." : "Exécuter la purge"} icon={I.trash('#fff')} color={P.red} onClick={handlePurge} disabled={saving || purgeConfirm !== 'SUPPRIMER'} /></div>
        </div>
      )}

      {tool === 'import' && (
        <div style={cardStyle}>
          <h3 style={{ color: '#3B6B8A', margin: '0 0 16px', fontSize: 14, fontWeight: 700 }}>Import OP depuis Excel</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div>
              <Label>1. Télécharger le canevas</Label>
          ...
          {importPreview && importExercice && <div style={{ marginTop: 20, textAlign: 'right' }}><ActionBtn label={saving ? "Import en cours..." : `Importer ${importPreview.total} OP`} icon={I.check()} color="#3B6B8A" onClick={handleImport} disabled={saving} /></div>}
        </div>
      )}

      {tool === 'compteurs' && (
        <div style={cardStyle}>
          <h3 style={{ color: P.gold, margin: '0 0 16px', fontSize: 14, fontWeight: 700 }}>Recaler les compteurs</h3>
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end' }}>
            <div><Label>Exercice à recaler</Label><select value={compteurExercice} onChange={e => setCompteurExercice(e.target.value)} style={{ ...styles.input, width: 200, marginBottom: 0, fontSize: 12 }}><option value="">-- Exercice --</option>{exercices.map(e => <option key={e.id} value={e.id}>{e.annee}</option>)}</select></div>
      ...
      {tool === 'doublons' && (
        <div style={cardStyle}>
          <h3 style={{ color: P.gold, margin: '0 0 16px', fontSize: 14, fontWeight: 700 }}>Doublons de numéros</h3>
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', marginBottom: 20 }}>
            <div><Label>Exercice à scanner</Label><select value={doublonExercice} onChange={e => setDoublonExercice(e.target.value)} style={{ ...styles.input, width: 200, marginBottom: 0, fontSize: 12 }}><option value="">-- Exercice --</option>{exercices.map(e => <option key={e.id} value={e.id}>{e.annee}</option>)}</select></div>
            <ActionBtn label="Scanner" icon={I.search('#fff')} color={P.gold} onClick={handleDetecterDoublons} disabled={!doublonExercice} />
          </div>
          {doublons !== null && (
            doublons.length === 0 ? <div style={{ padding: 12, background: P.greenLight, color: P.greenDark, borderRadius: 8, fontWeight: 600, fontSize: 12 }}>✓ Aucun doublon détecté</div> : (
              <div>
                ...
                {doublons.map((dup, i) => (
                  <div key={i} style={{ padding: 12, background: '#FAFAF8', border: `1px solid ${P.border}`, borderRadius: 8, marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontFamily: 'monospace', fontWeight: 800, fontSize: 13, marginBottom: 4 }}>{dup.numero} <Badge bg={P.goldLight} color={P.gold}>{dup.count}x</Badge></div>
                      <div style={{ fontSize: 10, color: P.textSec }}>{dup.ops.map(o => `${o.type} (${formatMontant(o.montant)}F)`).join(' | ')}</div>
                    </div>
