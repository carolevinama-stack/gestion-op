...
if(isNaN(m) || m === 0) { notify("error", "Erreur", "Veuillez saisir un montant valide."); return; }
  ...
  const handlePrintBordereau = (bt) => {
    ...
    const rows = btOps.map((op,i) => `<tr><td style="text-align:center">${i+1}</td><td style="width:150px">${escapeHtml(getBen(op))}</td><td>${escapeHtml(op.objet||'-')}</td><td style="text-align:center;font-family:monospace;font-size:9px;font-weight:bold">${escapeHtml(op.numero)}</td><td style="text-align:center">${nbEx}</td><td style="text-align:right;font-family:monospace;font-weight:bold">${Number(op.montant||0).toLocaleString('fr-FR')}</td></tr>`).join('');
