...
const MontantInput = ({ value, onChange, style, placeholder, disabled, ...rest }) => {
  ...
  const handleFocus = () => {
    setIsFocused(true);
    // Afficher le nombre brut sans séparateurs
  ...
  const handleChange = (e) => {
    ...
    setDisplayValue(val);
    // Envoyer la valeur brute au parent
...
<body>
  ...
  const PageConsulterOp = () => {
    ...
    // Charger un OP depuis consultOpData (venant d'une autre page)
    useEffect(() => {
      if (consultOpData) {
    ...
    // Dupliquer : pré-remplir le formulaire Nouvel OP
    const handleDuplicate = () => {
      if (!selectedOp) return;
      // On passe l'OP via consultOpData mais on va vers nouvelOp
      // On crée un objet spécial pour indiquer la duplication
